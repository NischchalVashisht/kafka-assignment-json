...
object ReadJson extends App {
 ...
 def getProduceData()= {
   ...
   val json = Source.fromFile("/home/knoldus/Downloads/kafka-assignment/src/main/resources/Student.json").getLines().mkString
   val parsedJson = net.liftweb.json.parse(json)
   val listOFStudent = parsedJson.children.map(list => list.extract[Student])
   listOFStudent
 }

 def writeToFile(listOfStudent:String)={
